...
struct Mincut : public Pass {
  ...
  void execute(std::vector<std::string> args, RTLIL::Design* design) override
  {
    ...
    log("Module Name = %s\n", module->name.c_str());
    size_t init_num_cells = module->cells().size();
    log("Cells = %zu\n", init_num_cells);
    ...
    for (auto cell : module->cells()) {
      for (auto conn : cell->connections()) {
        ...
        std::vector<hash_t> *target_cells = search_target_cell(module, cell->hash(), get_hash (conn.second));
    ...
    for (auto a : vec_grp_a) {
      for (auto cell : module->cells()) {
        if (cell->hash() == a) {
          std::string str = module->name.str() + "/" + cell->name.str();
          log ("%s\n", str.c_str());
          if (init) {
            Pass::call(design, "select " + module->name.str() + "/" + cell->name.str());
            init = false;
          } else {
            Pass::call(design, "select -add " + module->name.str() + "/" + cell->name.str());
          }
          break;
        }
      }
    ...
    log ("-------------------------------\n");
    // Pass::call(design, "select -list");
    log ("-------------------------------\n");
    Pass::call(design, "submod -name sub_0");
    ...
    Pass::call(design, "submod -name sub_1");
    log ("\n");
  }
